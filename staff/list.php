 ...
 *
 * USERNAME REMOVED:
 *   - data-username attribute on each card changed to data-email
 *   - $s['username'] pill display changed to $s['email']
 *   - .staff-card-username CSS class comment updated
 *   - JS filter updated: card.dataset.username → card.dataset.email
 *   - Search placeholder updated to "Search name or email…"
 *   - HTML comment on card data-* attributes updated
 *
 * @package EduSync
...
<style>
  ...
  .staff-card-name { font-weight: 600; font-size: .92rem; color: var(--text); line-height: 1.3; }

  /* Monospace pill showing the email address — CHANGED: was username */
  .staff-card-email {
  ...
  .staff-card-date { font-size: .72rem; color: var(--text-muted); }
  .staff-card-actions {
    display: flex;
    gap: 6px;
    width: 100%;
...
<div class="app-layout">
  ...
  <main class="content">
    ...
    <div class="dir-toolbar">
      <!-- Search: matches data-name and data-email — CHANGED: was data-username -->
      <div class="search-bar">
        <span>🔍</span>
        <input type="text" id="dirSearch" placeholder="Search name or email…">
        <!-- CHANGED: placeholder was "Search name or username…" -->
      </div>
    ...
    <!-- Live count updated by filterCards() -->
    ...
    <div class="staff-grid" id="staffGrid">
      ...
      <?php else: ?>
        ...
        <?php foreach ($staff as $s):
          // Build two-letter initials from full name
        ...
        >
          ...
          <div class="staff-avatar <?= $avatarClass[$s['role']] ?? 'avatar-blue' ?>">
            <?= $initials ?>
          </div>
          ...
          <!-- Email pill — CHANGED: was $s['username'] with class staff-card-username -->
          <div class="staff-card-email"><?= htmlspecialchars($s['email']) ?></div>

          <!-- Role badge, status badge, join date -->
          <div class="staff-card-meta">
            <span class="badge <?= $badgeClass[$s['role']] ?? 'badge-gray' ?>">
 ...
 *
 * USERNAME REMOVED:
 *   card.dataset.username changed to card.dataset.email
 *   Both the text match and the comment updated.
 */
(function () {
  ...
  function filterCards() {
    ...
    document.querySelectorAll('.staff-card').forEach(card => {
      // Text match: checks full name OR email — CHANGED: was card.dataset.username
      const matchQ      = !q      || card.dataset.name.includes(q)  || card.dataset.email.includes(q);
    ...
    countEl.textContent =
      'Showing ' + visible + ' staff member' + (visible !== 1 ? 's' : '');
  }
